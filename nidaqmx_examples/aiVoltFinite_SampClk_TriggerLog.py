...
with nidaqmx.Task(task_name) as task:
    ...
    data = numpy.zeros((number_of_channels, number_of_samples_per_channel), dtype=numpy.float64)
    dataPrint = numpy.zeros((number_of_samples_per_channel, number_of_channels), dtype=numpy.float64)

    task.start()
    # Python does not support t0 from the first sample of waveform, use current start time as t0
    ...
    dataPrint = numpy.transpose(data)
